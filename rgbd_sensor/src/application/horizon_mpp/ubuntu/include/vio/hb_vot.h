...
int HB_VOT_UpdatePara(uint8_t dev);
// 设置LCD背光亮度
...
int HB_VOT_BindVps(uint8_t vpsGroup, uint8_t vpsChn,
		uint8_t layer, uint8_t chn);
...
int HB_VOT_SetMipiDSILcp(uint8_t len, uint8_t *data);
int HB_VOT_SetMipiDSIS1p(uint8_t cmd, uint8_t data);
int HB_VOT_SetMipiDSISnp(uint8_t data);
int HB_VOT_InitMipiDSICore(VOT_MIPI_CORE_INIT_S *init_data);
