...
int HobotMipiCapIml::UpdateConfig(MIPI_CAP_INFO_ST &info) {
  ...
  int pym_layer = -1;
  try {
    src_width_ = root["pipeline0"]["pym"]["pym_ctrl"]["src_in_width"].asInt();
    src_height_ = root["pipeline0"]["pym"]["pym_ctrl"]["src_in_height"].asInt();
...
int HobotMipiCapImlJ5pi::initEnv(std::string sensor) {
...
// mipi sensor的信息数组
SENSOR_ID_T sensor_id_list_j5pi[] = {
  {1, 0x18, I2C_ADDR_8, 0x00, "ar0820"},  // ar0820
};

std::vector<std::string> HobotMipiCapImlJ5pi::listSensor() {
  std::vector<std::string> device;
  int i = 0;
  char cmd[256];
  char result[1024];
  ...
  SENSOR_ID_T *sensor_id_list = sensor_id_list_j5pi;
  for (i = 0; i < ARRAY_SIZE(sensor_id_list_j5pi); i++) {
...
int HobotMipiCapImlJ5pi::resetSensor(std::string sensor) {
  std::cout << "HobotMipiCapImlJ5pi::resetSensor" << std::endl;
